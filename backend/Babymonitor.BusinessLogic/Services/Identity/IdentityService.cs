...
namespace Babymonitor.BusinessLogic.Services.Identity;
...
public sealed class IdentityService : IIdentityService
{
    ...
    public async Task<string?> DisplayNameAsync(Guid userId) =>
        (await m_identityRepository.FindByIdAsync(userId))?.UserName;

    private async Task<OperationResult<Empty>> InviteExistingAccountAsync(
        Guid inviterId, string inviterName, BabymonitorUser invitee)
    {
    ...
    private async Task GrantUserRoleAsync(BabymonitorUser user)
    {
    ...
    /// </summary>
    private async Task SendConfirmationAsync(BabymonitorUser user)
    {
