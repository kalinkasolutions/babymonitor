...
public sealed class PairingService : IPairingService
{
        ...
        Guid userId, Guid? thisDeviceId, ClaimPairingDto dto)
    {
        ...
        if (token is null)
        {
            ...
            // holding the phone, and telling them apart would help anyone guessing at codes.
        ...
        m_logger.LogInformation(
            "Account {UserId} linked with {OtherUserId} via device {DeviceId}",
    ...
    public Task<OperationResult<Empty>> UnlinkAsync(Guid userId, Guid otherUserId)
    {
        return m_pairingRepository.UnlinkAccountsAsync(userId, otherUserId);
    }
