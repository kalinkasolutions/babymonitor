...
class PersistentCookieJarTest {
    ...
    /**
     * The backend marks the auth cookie Secure, and the debug build allows cleartext so a laptop
     * on the LAN works with no certificate. Filtering on the host alone sent a year-long session
     * cookie over that plain connection; the cookie's own attributes are what stop it.
     */
    @Test
    fun `a secure cookie is not sent over a plain connection`() {
        val store = InMemoryCookieStore()
        val year = NOW + 365L * 24 * 60 * 60 * 1000
        val jar = PersistentCookieJar(store, now = { NOW })
        jar.saveFromResponse(url, listOf(authCookie(year)))

        val overHttp = jar.loadForRequest("http://baby.kalinka-work.lqy.ch/api/auth/status".toHttpUrl())

        assertTrue(overHttp.isEmpty())
    }

    @Test
    fun `the same cookie still travels over https`() {
        val store = InMemoryCookieStore()
        val year = NOW + 365L * 24 * 60 * 60 * 1000
        val jar = PersistentCookieJar(store, now = { NOW })
        jar.saveFromResponse(url, listOf(authCookie(year)))
