...
import ch.lqy.babyphone.net.SignalResult
...
class CallCenter private constructor(private val context: Context) {
    ...
    val relayOnly: StateFlow<Boolean> = _relayOnly.asStateFlow()
    ...
    private fun listen() {
        ...
        scope.launch {
            stream.events.collect { event ->
                when (event) {
                    is DeviceEvent.Signal -> onSignal(event.signal)
        ...
        when (effect) {
            is CallEffect.Open -> {
                link = WebRtcMediaLink(
                    ...
                    relayOnly = callPreferences.relayOnly,
    ...
    fun setRelayOnly(value: Boolean) {
        callPreferences.relayOnly = value
        _relayOnly.value = value
    }
