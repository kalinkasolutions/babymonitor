...
class WebRtcMediaLink(
    ...
    init {
        ...
        val configuration = PeerConnection.RTCConfiguration(iceServers.toIceServers()).apply {
            ...
            // Stated rather than left to the default, because the alternative — forcing the relay
            // — would put a server in the middle of every night at home.
    ...
    private fun openMicrophone() {
        ...
        val track = factory.createAudioTrack("babyphone-audio", source)
    ...
    private fun openCamera(capturing: Boolean) {
        ...
        val name = enumerator.deviceNames.firstOrNull(enumerator::isFrontFacing)
            ...
            ?: return listener.onFailed("This phone has no camera to send.")

        val capturer = enumerator.createCapturer(name, null)
        val helper = SurfaceTextureHelper.create("babyphone-capture", WebRtcEgl.base.eglBaseContext)
        ...
        val track = factory.createVideoTrack("babyphone-video", source)
        connection?.addTrack(track, listOf(StreamId))
    ...
    override fun createOffer() {
        ...
        connection.createOffer(
            describing { sdp ->
                connection.setLocalDescription(ignoring(), sdp)
        ...
        connection.setRemoteDescription(
            settled {
                connection.createAnswer(
                    describing { answer ->
                        connection.setLocalDescription(ignoring(), answer)
    ...
    override fun acceptAnswer(sdp: String) {
        connection?.setRemoteDescription(ignoring(), SessionDescription(SessionDescription.Type.ANSWER, sdp))
    }
...
private fun describing(onCreated: (SessionDescription) -> Unit) = object : SdpObserver {
...
private fun settled(onSet: () -> Unit) = object : SdpObserver {
...
private fun ignoring() = object : SdpObserver {
    override fun onCreateSuccess(description: SessionDescription?) = Unit
    override fun onSetSuccess() = Unit
    override fun onCreateFailure(error: String?) = Unit
    override fun onSetFailure(error: String?) = Unit
