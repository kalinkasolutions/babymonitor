...
fun LoginScreen(
    ...
    val canSubmit = !busy && email.isNotBlank() && password.isNotBlank() && hasServer
    ...
    // Registering talks to the server too, so it carries the typed address across with it.
    fun goToRegister() {
        onServerUrlChange(serverUrl)
        onGoToRegister()
    }

    AuthScaffold(title = "Babymonitor", subtitle = "Sign in to pair this phone.") {
        OutlinedTextField(
            value = serverUrl,
        ...
        TextButton(onClick = ::goToRegister, enabled = !busy && hasServer) {
